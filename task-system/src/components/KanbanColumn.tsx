  ...
  return (
    <div
      ...
      className={`flex flex-col gap-2 transition-all duration-200 h-fit ${
        variant === "kanban" 
          ? `rounded-[20px] !p-4 w-full min-w-[280px] min-h-[200px] border ${
    ...
    >
      <div className={`flex items-center justify-between ${variant === "list" ? "!p-2 !pt-6 !pb-2 mb-1 border-b-2 border-[var(--border)]/50" : "p-1"}`}>
      ...
      <SortableContext items={tasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
        <div className={`flex flex-col ${variant === "kanban" ? "gap-3" : "gap-0"}`}>
          {tasks.map((task) => (
            variant === "kanban" ? (
              <TaskCard
                key={task.id}
